#' @name get_results
#' @title POST file to the GraphQL API.
#'
...
#' @name dg_call
#' @title Call the Datenguide GraphQL API
#'
#' @description Makes a call to the Datenguide GraphQL API and returns the results.
#'
...
#'   Please see `?dg_descriptions` for a full list.
#' @param year Numeric year(s) for which you want to retrieve the data.
#' @param substat_name Character string containing the name of the
#'   sub-statistic. Please see `?dg_descriptions` for a full list. Defaults to
#'   all available sub-statistics if not specified.
#' @param parameter Character string containing the name(s) of the parameter(s)
#'   you want to retrieve. Please see `?dg_descriptions` for a full list.
#'   Defaults to all available parameters if not specified.
...
#' @param parent_chr Character string containing the ID of a super-region to
#'   which query should be restricted. Should be a region of higher order than
#'   `nuts_nr` oder `lau_nr`.
...
dg_call <- function(region_id = NULL,
  ...
  if (!all_regions) {
    api_results <- get_results(
      field = field, substat_name = substat_name,
      stat_name = stat_name
    ) %>%
      clean_region()
