...
dg_map <- function(lookup = "GADM3",
  ...
  if(nuts_nr == 2){
    ...
    utils::download.file('https://daten.gdz.bkg.bund.de/produkte/vg/nuts250_1231/aktuell/nuts250_12-31.utm32s.shape.zip',
                  temp)
  ...
  if(nuts_nr == 1){
    dg_dat <- dg_call(stat_name = stat_name,
                      year = year,
